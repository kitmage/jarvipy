...
class ObjectDetector:
    ...
    def detect(self, image_tensor: object) -> list[Detection]:
        ...
        classes = cast(
            list[list[float]],
            self._interpreter.get_tensor(self._output_details[1]["index"]),
        )[0]
        scores = cast(
            list[list[float]],
            self._interpreter.get_tensor(self._output_details[2]["index"]),
        )[0]
        ...
        for bbox, class_id, score in zip(boxes, classes, scores):
            ...
            detections.append(
                Detection(
                    label=label, confidence=confidence, bbox=(xmin, ymin, xmax, ymax)
                )
            )
        return detections
...
def contains_person_or_vehicle(detections: list[Detection]) -> bool:
    ...
    return any(
        det.label == "person" or det.label in VEHICLE_CLASSES for det in detections
    )
